...
class KGQueryHandler implements Handler<RoutingContext> {
	
	private final static Logger log = LoggerFactory.getLogger(KGQueryHandler.class)
	
	static JsonSlurper jsonParser = new JsonSlurper()
		
	static Vertx vertxInstance
		
	public KGQueryHandler(Vertx vertxInstance) {
	...
	public void handle(RoutingContext ctx) {
		...
		ctx.request().bodyHandler({ body ->
			...
			try {
				...
				vertxInstance.eventBus().send("kgquery", requestJSON, options) { Future<Message> response ->
					...
					if(response.succeeded()) {
						...
						String message = 'error in kgquery request.'
				...
				String message = 'KGQueryHandle Error: ' + ex.localizedMessage
