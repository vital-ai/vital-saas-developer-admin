...
class ToolHandler implements Handler<RoutingContext> {
	
	private final static Logger log = LoggerFactory.getLogger(ToolHandler.class)
	
	static JsonSlurper jsonParser = new JsonSlurper()
		
	static Vertx vertxInstance
		
	public ToolHandler(Vertx vertxInstance) {
	...
	public void handle(RoutingContext ctx) {
		...
		ctx.request().bodyHandler({ body ->
			...
			try {
				...
				vertxInstance.eventBus().send("tool", requestJSON, options) { Future<Message> response ->
					...
					if(response.succeeded()) {
						...
						String message = 'error in tool request.'
				...
				String message = 'ToolHandle Error: ' + ex.localizedMessage
